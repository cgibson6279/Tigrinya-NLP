...
def main():
    ...
    #add a sys arg here later
    text_path = "C:/Users/camer/Dropbox/CUNY/Tigrinya-NLP/data/texts/eng-tig-dict.txt"
    eng_tig_dict = dict()

    #open arg with sys arg later
        ...
        for i in range(len(tig_dict)):
            ...
            if ',' in line[2]:
                ...
                for word in line[2]:
                    ...
                    if not word in eng_tig_dict[line[0].replace('-','').strip()]["words"]:
                        eng_tig_dict[line[0].replace('-','').strip()]["words"][word.strip()] = x.strip()
            else:
                x, y, z = tlr.ti_transliterator(line[2])
                eng_tig_dict[line[0].replace('-', '').strip()]["words"][line[2].strip()] = x.strip()

        json_object = json.dumps(eng_tig_dict, ensure_ascii=False, indent = 4).encode('utf8')
